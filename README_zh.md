...
GTM 的全称是 `Global Transaction Manager`，是一个解决分布式事务问题的框架，基于 Go 编写。GTM 的原理基于 2PC 协议，但进行了很多优化，主要意在提升易用性，降低业务的接入成本。GTM 允许业务实现相比标准 2PC 更少的接口，尤其是回滚接口。基于长期在交易类系统上的实践经验，以及对大量业务场景的总结思考，我们发现，在参与者执行顺序确定的前提下，很多时候业务并不需要实现回滚接口。本质上，GTM 是 2PC 的超集，可以实现 2PC 的所有场景。
